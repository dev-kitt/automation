...
function gppLogin() {
  ...
  // login
  Sys.Browser("*").Page("*").ToUrl(Project.Variables.gppURL);
  var gpp = Aliases.Browser.gppFinastra_Main;
  if (equal(gpp.Auth.Exists, true)) {
...
function gppLogout() { 
  ...
  // verify
  aqObject.CheckProperty(gpp.Auth, "Exists", cmpEqual, true);  
...
{                   
  ...
  if (equal(Test, Start)) {
    Log.Message("TEST: failed to launch website on: " + aqTestCase.CurrentTestCase.Name);
    Runner.Stop();
  }
}
